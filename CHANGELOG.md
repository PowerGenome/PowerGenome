...
- Simultaneous GenX and Macro output from a single `run_powergenome` call. GenX remains the default output format (`genx_output` defaults to `true`), and enabling Macro no longer disables GenX: write both formats at once with the `--macro --genx` flags or `macro_output: true` + `genx_output: true` settings. Set `genx_output: false` (with Macro on) for Macro-only output. The `--macro`/`--genx` flags and `macro_output`/`genx_output` boolean settings are all case-insensitive (`--Macro`/`--GENX`; `"True"`/`"TRUE"` values). Writing both formats in one run reuses the shared data processing and is faster than running PowerGenome once per model.
