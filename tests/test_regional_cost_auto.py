...
def test_auto_create_technology_map(regional_cost_settings):
    ...
    new_gen_df = pd.DataFrame({
        ...
        "tech_detail": ["1-on-1 Combined Cycle (H-Frame)", "Class3", "Nuclear - Large"],
...
def test_validate_cost_coverage_success(regional_cost_settings):
    ...
    # Create tech map
    tech_map = {
        "NaturalGas_1-on-1 Combined Cycle (H-Frame)_Moderate": "CC - single shaft",
...
def test_modified_resources_validation(regional_cost_settings, caplog):
    ...
    assert "Modified resource 'CustomTech'" in caplog.text or "CustomNG_Modified_Advanced" in caplog.text
