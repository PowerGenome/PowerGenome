...
@auto_fill_settings()
def load_policy_scenarios(settings: dict = None) -> pd.DataFrame:
    ...
    if policies.duplicated(subset=["case_id", "year", "region"]).any():
        ...
        warned = True
    # If the user's case_id is not in the policies file, use the "all" values
    if settings["case_id"] not in policies["case_id"].astype(str).str.lower().to_list():
        policies.loc[
